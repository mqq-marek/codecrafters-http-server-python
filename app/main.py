...
def user_agent(request, *params):
    ...
    response = Response(
        request, response_code=200, headers={"Content-Type": "text/plain"}, body=agent
    )
    return response
...
def post_files(request, *params):
    ...
    if params:
        ...
        with open(path_to_file, "wb") as f:
            f.write(body)
        response = Response(request, response_code=201)
    return response


if __name__ == "__main__":
    app = WebServerApp(
        host=arguments.values.get("host"), port=arguments.values.get("port")
    )
    app.run()
